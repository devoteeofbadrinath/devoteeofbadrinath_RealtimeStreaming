...
if __name__ == "__main__":
    with application_context() as ctx:
        ...
        logger.info("Initial: startring_offsets: %s", starting_offsets)
        ...
        logger.info("sr_schema = %s",sr_schema)
        query = read_from_kafka(
            ctx.config,ctx.spark,starting_offsets,
            partial(process_stream_batch, ctx, sr_schema)
        )
