...
def read_kafka_offsets(spark: SparkSession, conf: StreamConfig) -> Dict[str, int]:
    """
    Retrive the latest offsets from Pheonix table for a specified kafka topic.
    
    :param spark: The SparkSession object.
    :param conf: Stream config variables.
    :return: A dictionary mapping topic partitions to their offsets.
    """
    table = f"{conf.pheonix_data_source['generic_schema']}.{conf.pheonix_data_source['offset_table']}"
    offsets_df = read_from_pheonix(
        spark,
        table,
        conf.pheonix_data_source["zkurl"]
    )
    offsets = offsets_df.select('topic_partition', 'topic_offset') \
        .filter(F.col('topic_name') == conf.kafka_topic)
    offsets = {r.topic_partitions: r.topic_offset for r in offsets.collect()}
    if not offsets:
...
def write_offsets(
    ...
    and writes it to the specified Pheonix table.
    It ensures that the offsets are recorded
    in the Pheonix table for future reference.
    ...
    if latest_offsets:
        ...
        write_to_pheonix(
            offset_df,
            f"{ctx.config.pheonix_data_source['generic_schema']}."
            f"{ctx.config.pheonix_data_source['offset_table']}",
            ctx.config.pheonix_data_source["zkurl"]
        )
...
def write_metrics(ctx: ApplicationContext, metrics: RecordMetrics):
    ...
    if metrics.total > 0:
        ...
        write_to_pheonix(
            stats_df,
            f"{conf.pheonix_data_source['generic_schema']}."
            f"{conf.pheonix_data_source['stats_table']}",
            conf.pheonix_data_source["zkurl"]
        )
